...
def home(request):
    return JsonResponse({
        ...
        "endpoints": {
            ...
            "list_customers": "/api/hello/",
            "customer_detail": "/api/customer/<id>/",
            "update_status": "/api/update_specific_date/",
            "date_status": "/api/customer/<id>/date-status/?date=YYYY-MM-DD",
             "generate_pdf": "/api/customer/<id>/pdf/?month=YYYY-MM"
        }
    })
...
def customer_stats(request, id):
    try:
        ...
        # Active days = days customer was present in this month
...
# Optimized: Save ONLY missed meals
@csrf_exempt
def update_specific_date(request):
    ...
    try:
        ...
        # ----- LUNCH -----
        if lunch:
            # taken → delete record (default is taken)
...
# Helper function to update monthly cache
def update_monthly_cache(customer, target_date):
    """Update cached monthly statistics"""
    today = date.today()
    
    # Only update if target date is in current month
